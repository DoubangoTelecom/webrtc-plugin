...
public:
	...
	ExMediaTrackConstraints(std::shared_ptr<MediaTrackConstraintSet> exact = nullptr, std::shared_ptr<MediaTrackConstraintSet> ideal = nullptr);
	...
	_inline const std::shared_ptr<MediaTrackConstraintSet> exact()const { return m_exact; }
	_inline const std::shared_ptr<MediaTrackConstraintSet> ideal()const { return m_ideal; }
...
#pragma warning(disable:4251)
	...
	std::shared_ptr<MediaTrackConstraintSet> m_exact; // only if type is ExMediaTrackConstraintsTypeComposite
	std::shared_ptr<MediaTrackConstraintSet> m_ideal; // only if type is ExMediaTrackConstraintsTypeComposite
#if _MSC_VER
#pragma warning(pop)
#endif
};
